...
@section('main-content')
 ...
 <div class="card shadow mb-4">
    ...
    <div class="card-header py-3 align">
      ...
      <select class="form-control float-left col-md-2" onchange="myFunction({{\APP\Task::getAllMyTasksId(Auth()->user()->name)}}, {{\APP\Task::getAllMyOverDueId(Auth()->user()->name)}}, {{\APP\Task::getMyOpenId(Auth()->user()->name)}});" id="selDepartment">        
      ...
      <select class="form-control float-left col-md-2" onchange="myFunction({{\APP\Task::getAllMyTasksId(Auth()->user()->name)}}, {{\APP\Task::getAllMyOverDueId(Auth()->user()->name)}}, {{\APP\Task::getMyOpenId(Auth()->user()->name)}});" id="selMyTask">
        <option>Select Task</option>
        <option>All My tasks</option>
        <option>My Overdue tasks</option>
        <option>My open Tasks</option>
      </select>
      <select class="form-control float-left col-md-2" onchange="myFunction({{\APP\Task::getAllMyTasksId(Auth()->user()->name)}}, {{\APP\Task::getAllMyOverDueId(Auth()->user()->name)}}, {{\APP\Task::getMyOpenId(Auth()->user()->name)}});" id="selType">
        <option value="access">Access</option>
        <option value="public">Public</option>
        <option value="private">Private</option>
      </select>
      <a href="{{route('task.create')}}" class="btn btn-primary btn-sm float-right" data-toggle="tooltip" data-placement="bottom" title="Add User"><i class="fas fa-plus"></i> Create Task</a>
    </div>
    <div class="card-body">
          ...
          <tbody>
            @foreach($tasks as $task)   
                <tr>
                    ...
                    <td>
                        @if($task->assignedTo==Auth()->user()->name)
                          <form method="POST" action="{{route('task.edit', $task->id)}}">
                            @csrf
                            <button type="submit"><i class="fas fa-book"></i></button>
                          </form>
                        @endif
                        @if($task->assignedTo=="" && $task->createdBy != Auth()->user()->name)
                          <form method="POST" action="{{route('task.assign', $task->id)}}">
                          @csrf
